...
def add_event(date, text):
    ...
    save_events(events)
    print(f"Added: {date} - {text} {' '.join('#'+t for t in tags)}")
...
def mark_done(date, index):
    ...
    save_events(events)
    print(f"Marked as done: {events[date][index - 1]['text']}")
...
def list_events(*args):
        ...
        for d in sorted(events.keys()):
            ...
            for ev in filtered:
                if not found:
                    ...
                    found = True
                mark = "[x]" if ev["done"] else "[ ]"
                print(f"{d} {mark} {ev['text']}")
        if not found:
    ...
    if re.match(r"\d{4}-\d{2}-\d{2}", arg1):
        ...
        if not include_past and not is_future_or_today(date_str):
            print("Date is in the past. Use 'past' or 'all' to include it.")
            return
        if date_str in events:
            evs = filter_events(events[date_str], status)
            if not evs:
    ...
    print("Invalid argument. Type 'help' for usage.")

def help_menu():
    print("""
Commands:
  add YYYY-MM-DD "text" [#tag1 #tag2]
  list [day | YYYY-MM | YYYY-MM-DD | #tag] [pending|done|all] [past]
  done YYYY-MM-DD N
  help
  exit
